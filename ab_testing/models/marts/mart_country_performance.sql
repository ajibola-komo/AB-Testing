...
count(case when t.is_converted = 1 then t.user_id end)::FLOAT / nullif(count(t.user_id),0) as session_conversion_rate,
avg(t.duration_seconds) as average_session_duration
from {{ref('mart_countries')}} s inner join {{ref('mart_ab_test')}} t
